...
public class BattleManager : MonoBehaviour
{
    ...
    /// <summary>バトルシーンの背景画像を表示するUI</summary>
    ...
    void Awake()
    {
        ...
        {
            backgroundRenderer.sprite = currentStage.GetBackgroundSprite();
        }

        // BGM再生
        AudioManager.Instance.PlayBGM(currentStage.GetStageBGM());
    ...
    /// <summary>
    /// Fireballクリック時の効果処理。
    /// 効果タイプに応じて現在フェーズの設定値を参照し、
    /// ダメージまたは回復を適用する。
    /// </summary>
    /// <param name="effectType"> Fireballの効果の種類を保持</param>
    private void HandleFireballEffect(FireballEffectType effectType)
    {
        ...
        var battleSessionData = BattleSessionData.Instance;
        var currentStage = battleSessionData.GetCurrentStage();
        var currentPhase = currentStage.GetStagePhases()[battleSessionData.GetCurrentPhaseIndex()];

        // 決定されたFireballのタイプ別にその効果を発動する
        switch (effectType)
        {
            case FireballEffectType.DamageToAllEnemy:  // 敵全員に攻撃を与える効果
                enemyAreaManager.TakeDamageToAll(currentPhase.GetFireballDamageToEnemyAmount());
                break;
            case FireballEffectType.DamageToAlly:  // 味方(プレイヤー)に攻撃を与える効果
                allyAreaManager.TakeDamageToSharedHP(currentPhase.GetFireballDamageToAllyAmount());
                break;
            case FireballEffectType.HealToAlly:        // 味方(プレイヤー)のHPを回復する効果
                allyAreaManager.HealSharedHP(currentPhase.GetFireballHealToAllyAmount());
                break;
        }
