...
/// <summary>
/// 敵モンスター配置エリアの管理クラス。
/// 敵モンスターの生成、ターゲット選択、ダメージ処理、継続ダメージ管理などを担当。
/// </summary>
public class EnemyAreaManager : MonsterAreaManager
{
    ...
    public void PrepareEnemyAttackDamages()
    {
        ...
        foreach (var enemyMonster in spawnedMonsters)
        {
            ...
            // [attackPowe-10, attackPoewr+10]の範囲でランダムに「味方HPに与えるダメージ量(attackPowerAmount)」を決める。
            int baseAttackPower = enemyMonster.GetAttackPower();
            int attackPowerAmount = UnityEngine.Random.Range(baseAttackPower - 10, baseAttackPower + 11);
